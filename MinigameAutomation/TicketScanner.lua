...
local function updateAttemptsUI()
	...
	if not mainInterface or not State.gui then
		if State.gui then
			State.gui.attemptsLabel.Text = "⚠ 0 Attempts"
		end
		return
	end
	...
	if minigameTicket then
		...
		if button then
			...
			if itemAmount then
				...
				State.gui.attemptsLabel.Text = "⚠ " .. count .. " Attempts"
				return
			end
		end
	end
